...
pub fn ScrollCallback(window: *glfw.GLFWwindow, xoffset: f64, yoffset: f64) callconv(.C) void {
    ...
    const io = imgui.GetIO();
    io.AddMouseWheelEvent(@as(f32, xoffset), @as(f32, yoffset));
}
...
pub fn CursorPosCallback(window: *glfw.GLFWwindow, x: f64, y: f64) callconv(.C) void {
    ...
    const io = imgui.GetIO();
    io.AddMousePosEvent(@as(f32, x), @as(f32, y));
    bd.LastValidMousePos = .{ .x = @as(f32, x), .y = @as(f32, y) };
}
...
fn UpdateMouseData() void {
    ...
    if (is_app_focused) {
        ...
        if (is_app_focused and bd.MouseWindow == null) {
            ...
            io.AddMousePosEvent(@as(f32, mouse_x), @as(f32, mouse_y));
            bd.LastValidMousePos = .{ .x = @as(f32, mouse_x), .y = @as(f32, mouse_y) };
        }
    }
}
...
fn UpdateMouseCursor() void {
    ...
    if (imgui_cursor == .None or io.MouseDrawCursor) {
        ...
        glfw.glfwSetCursor(bd.Window.?, bd.MouseCursors[@as(usize, @intFromEnum(imgui_cursor))] orelse bd.MouseCursors[@as(usize, @intFromEnum(imgui.MouseCursor.Arrow))]);
...
pub fn NewFrame() void {
    ...
    io.DeltaTime = if (bd.Time > 0) @as(f32, @floatFromInt(current_time - bd.Time)) else (1.0 / 60.0);
