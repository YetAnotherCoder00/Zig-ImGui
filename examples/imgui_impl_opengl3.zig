...
fn GetBackendData() ?*Data {
    return if (imgui.GetCurrentContext() != null) @ptrCast(@alignCast(imgui.GetIO().BackendRendererUserData)) else null;
}
...
pub fn Init(glsl_version_opt: ?[]const u8) bool {
    ...
    const bd: *Data = @ptrCast(@alignCast(imgui.MemAlloc(@sizeOf(Data))));
    bd.* = .{};
    ...
    bd.GlVersion = @as(gl.GLuint, major * 100 + minor);
    ...
    if (MAY_HAVE_EXTENSIONS) {
        ...
        while (i < extensions) : (i += 1) {
            if (gl.glGetStringi(gl.GL_EXTENSIONS, @as(gl.GLuint, i))) |ext_nt| {
...
fn SetupRenderState(draw_data: *imgui.DrawData, fb_width: c_int, fb_height: c_int, vertex_array_object: gl.GLuint) void {
    ...
    gl.glEnableVertexAttribArray(@as(c_uint, bd.AttribLocationVtxPos));
    gl.glEnableVertexAttribArray(@as(c_uint, bd.AttribLocationVtxUV));
    gl.glEnableVertexAttribArray(@as(c_uint, bd.AttribLocationVtxColor));
    gl.glVertexAttribPointer(
        @as(c_uint, bd.AttribLocationVtxPos),
        2,
        gl.GL_FLOAT,
        gl.GL_FALSE,
    ...
    gl.glVertexAttribPointer(
        @as(c_uint, bd.AttribLocationVtxUV),
        2,
        gl.GL_FLOAT,
        gl.GL_FALSE,
        @sizeOf(imgui.DrawVert),
    ...
    gl.glVertexAttribPointer(
        @as(c_uint, bd.AttribLocationVtxColor),
        4,
        gl.GL_UNSIGNED_BYTE,
        gl.GL_TRUE,
        @sizeOf(imgui.DrawVert),
...
pub fn RenderDrawData(draw_data: *imgui.DrawData) void {
    ...
    // Backup GL state
    const last_active_texture = @as(gl.GLenum, getGLInt(gl.GL_ACTIVE_TEXTURE));
    ...
    const last_blend_src_rgb = @as(gl.GLenum, getGLInt(gl.GL_BLEND_SRC_RGB));
    const last_blend_dst_rgb = @as(gl.GLenum, getGLInt(gl.GL_BLEND_DST_RGB));
    const last_blend_src_alpha = @as(gl.GLenum, getGLInt(gl.GL_BLEND_SRC_ALPHA));
    const last_blend_dst_alpha = @as(gl.GLenum, getGLInt(gl.GL_BLEND_DST_ALPHA));
    const last_blend_equation_rgb = @as(gl.GLenum, getGLInt(gl.GL_BLEND_EQUATION_RGB));
    const last_blend_equation_alpha = @as(gl.GLenum, getGLInt(gl.GL_BLEND_EQUATION_ALPHA));
    ...
    if (draw_data.CmdListsCount > 0) {
        for (draw_data.CmdLists.?[0..@as(usize, draw_data.CmdListsCount)]) |cmd_list| {
            ...
            for (cmd_list.CmdBuffer.items()) |pcmd| {
                if (pcmd.UserCallback) |fnPtr| {
                    ...
                    // Bind texture, Draw
                    gl.glBindTexture(gl.GL_TEXTURE_2D, @as(gl.GLuint, @intFromPtr(pcmd.GetTexID())));
                    if (MAY_HAVE_VTX_OFFSET and bd.GlVersion >= 320) {
                        gl.glDrawElementsBaseVertex(
                            gl.GL_TRIANGLES,
                            @as(gl.GLsizei, pcmd.ElemCount),
    ...
    gl.glUseProgram(@as(c_uint, last_program));
    gl.glBindTexture(gl.GL_TEXTURE_2D, @as(c_uint, last_texture));
    if (MAY_HAVE_BIND_SAMPLER and bd.GlVersion >= 330) gl.glBindSampler(0, last_sampler);
    gl.glActiveTexture(last_active_texture);
    gl.glBindVertexArray(@as(c_uint, last_vertex_array_object));
    gl.glBindBuffer(gl.GL_ARRAY_BUFFER, @as(c_uint, last_array_buffer));
    ...
    if (HAS_POLYGON_MODE) gl.glPolygonMode(gl.GL_FRONT_AND_BACK, @as(gl.GLenum, last_polygon_mode[0]));
...
fn CheckShader(handle: gl.GLuint, desc: []const u8) bool {
    ...
    if (log_length > 1) {
        ...
        defer buf.deinit();
        buf.resize_undefined(@as(u32, log_length + 1));
        gl.glGetShaderInfoLog(handle, log_length, null, @as([*]gl.GLchar, @ptrCast(buf.Data.?)));
        std.debug.print("{s}\n", .{buf.items()});
    }
...
fn CheckProgram(handle: gl.GLuint, desc: []const u8) bool {
    ...
    if (log_length > 1) {
        ...
        defer buf.deinit();
        buf.resize_undefined(@as(u32, log_length + 1));
        gl.glGetProgramInfoLog(handle, log_length, null, @as([*]gl.GLchar, @ptrCast(buf.Data.?)));
        std.debug.print("{s}\n", .{buf.items()});
    }
    return status != gl.GL_FALSE;
}
...
fn CreateDeviceObjects() bool {
    ...
    // Create shaders
    const version_str = @as([*:0]const u8, @ptrCast(&bd.GlslVersionString));
    ...
    gl.glBindTexture(gl.GL_TEXTURE_2D, @as(c_uint, last_texture));
    gl.glBindBuffer(gl.GL_ARRAY_BUFFER, @as(c_uint, last_array_buffer));
    gl.glBindVertexArray(@as(c_uint, last_vertex_array));
